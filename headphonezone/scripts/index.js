...
document.getElementById("slideshow").innerHTML = slideshow(slideshow_content[0].image1, slideshow_content[0].h, slideshow_content[0].p);
...
setInterval(() => {
   ...
   box.innerHTML = slideshow(slideshow_content[i].image1, slideshow_content[i].h, slideshow_content[i].p);
...
//View more to move to products page------------------------------------------>
let Viewmore= document.getElementById("Viewmore");
Viewmore.addEventListener("click",()=>{window.location.href="earphone.html"})
// brand logo's ---------------------------------------------------------------->
...
var arr=JSON.parse(localStorage.getItem('userCredentials'))||[]
console.log(arr[arr.length-1])

var user_name=document.getElementById('account')
if(arr.length>0)
user_name.innerText=arr[arr.length-1].first_name+' '+arr[arr.length-1].last_name;
// import {dataexport} from "./scripts/dataexport.js";
