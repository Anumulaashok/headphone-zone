...
// --------------filtering---------------------------------------->
let under1000=document.getElementById("banners").children;

for(let i=0;i<under1000.length;i++){
    under1000[i].addEventListener("click",filter);
}
function filter(){
    let rupe=this.id.split("r");
    let rupenum=+rupe[1];
  
   let appenddata= ProductsArray.filter((el)=>{
    let pricess=el.price.split(",").join("")
    pricess=+pricess
    console.log(pricess,rupenum);
        return pricess<=rupenum;
    })
